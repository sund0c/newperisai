...
@section('page-title', 'Dashboard Support')
...
@section('content')
    @php
        ...
        $validated = \App\Models\Report::where('status', 'validated')->count();
        $certificate = \App\Models\Report::where('status', 'certificate')->count();
        $closed = \App\Models\Report::where('status', 'closed')->count();
        $recent = \App\Models\Report::with(['reporter', 'latestStatusLog'])
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <a href="{{ route('support.reports.index', ['status' => 'submitted']) }}"
        ...
        <a href="{{ route('support.reports.index', ['status' => 'validated']) }}"
        ...
        <a href="{{ route('support.reports.index', ['status' => 'closed']) }}"
    ...
    {{-- Tiket Masuk Hari Ini --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
                <h3 class="text-base font-semibold text-gray-900">Tiket Masuk Hari Ini</h3>
                <p class="text-xs text-gray-400 mt-0.5">{{ today()->format('d M Y') }}</p>
            </div>
            <a href="{{ route('support.reports.index') }}" class="text-sm text-blue-600 hover:underline">Lihat semua</a>
        </div>

        @if ($recent->isEmpty())
            <div class="px-6 py-10 text-center">
                <p class="text-sm text-gray-400">Belum ada tiket masuk hari ini.</p>
            </div>
        @else
                        ...
                        @foreach ($recent as $report)
                            ...
                            <tr class="hover:bg-gray-50 transition-colors {{ $isClosed ? 'opacity-60' : '' }}">
                                ...
                                <td class="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
                                    {{ $latestLog ? $latestLog->created_at->format('d M Y') : '-' }} WITA
                                </td>
                                <td class="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">
