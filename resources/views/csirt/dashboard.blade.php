...
@section('page-title', 'Dashboard CSIRT')
@section('page-subtitle', 'Panel Mitigasi CSIRT Provinsi Bali')

@section('content')
    @php
        $notified = \App\Models\CsirtProcess::where('status', 'notified')->count();
        $inProgress = \App\Models\CsirtProcess::where('status', 'in_progress')->count();
        $closed = \App\Models\CsirtProcess::where('status', 'closed')->count();
        $total = \App\Models\CsirtProcess::count();
        $recent = \App\Models\CsirtProcess::with(['report', 'handler'])
            ->latest()
            ->limit(8)
            ->get();
    @endphp
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <a href="{{ route('csirt.reports.index', ['status' => 'notified']) }}"
        ...
        <a href="{{ route('csirt.reports.index', ['status' => 'in_progress']) }}"
    ...
    {{-- Tiket Mitigasi Terbaru --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-base font-semibold text-gray-900">Tiket Terbaru</h3>
            <a href="{{ route('csirt.reports.index') }}" class="text-sm text-indigo-600 hover:underline">Lihat semua</a>
        </div>

        @if ($recent->isEmpty())
            <p class="text-sm text-gray-400 text-center py-8">Belum ada tiket mitigasi.</p>
        @else
            <div class="divide-y divide-gray-50">
                @foreach ($recent as $process)
                    ...
                    @endphp
                    <a href="{{ route('csirt.reports.show', $process) }}"
