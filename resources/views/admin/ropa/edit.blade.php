    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div id="tab-pengamanan" class="tab-panel hidden space-y-5">
                ...
                @php
                    $pengamananFields = [
                        'technical_security_controls'        => ['Technical Security Controls',        'Kontrol teknis yang sudah diterapkan — enkripsi, autentikasi, audit log, firewall, dll.'],
                        'privacy_governance_controls'        => ['Privacy Governance Controls',        'Kontrol tata kelola privasi — kebijakan, purpose limitation, DSAR, data breach procedure.'],
                        'organizational_governance_controls' => ['Organizational Governance Controls', 'Kontrol tata kelola organisasi — SOP, pelatihan, NDA, perjanjian kerahasiaan.'],
                    ];
                @endphp

                @foreach ($pengamananFields as $fieldKey => [$fieldLabel, $fieldDesc])
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ $fieldLabel }}</label>
                            <p class="text-xs text-gray-400 mt-0.5">{{ $fieldDesc }}</p>
                        </div>
                        @if ($isEditable)
                        <button type="button"
                            onclick="addOrderedItem('ol-{{ $fieldKey }}')"
                            class="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 font-medium flex-shrink-0 ml-4">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            Tambah
                    ...
                    <div id="ol-{{ $fieldKey }}"
                        data-field="{{ $fieldKey }}"
                        data-old="{{ json_encode(old($fieldKey, $ropaActivity->$fieldKey ?? [])) }}"
                        data-readonly="{{ $isEditable ? 'false' : 'true' }}"
                        class="ol-container mt-2">
                    </div>
                </div>
                @endforeach
                ...
                <div class="border-t border-gray-100 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        @foreach (\App\Models\RopaRiskIndicator::LABELS as $val => $label)
                            ...
                            @endphp
                            <label class="flex items-start gap-2 {{ ($isDataSpesifik || !$isEditable) ? 'opacity-75' : 'cursor-pointer' }}">
                                <input type="checkbox"
                                    name="indikator_risiko[]"
                                    value="{{ $val }}"
