...
@section('page-subtitle', $ropaActivity->kode . ' · ' . ($ropaActivity->opd?->namaopd ?? '-') . ' · Tahun ' .
    ($ropaActivity->tahunAktif?->tahun ?? '-'))
...
@section('content')
    ...
    @php
        $tabs = [
            'umum' => 'Informasi Umum',
            'data' => 'Data Pribadi',
            'penyimpanan' => 'Penyimpanan & Pemrosesan',
            'pengamanan' => 'Pengamanan',
            'penerima' => 'Penerima Data',
            'risiko' => 'Hak & Risiko',
        ];
        $selectedUmum = $ropaActivity->personalDataTypes->where('is_spesifik', false)->pluck('jenis_data')->toArray();
        $selectedSpesifik = $ropaActivity->personalDataTypes
            ->where('is_spesifik', true)
            ->pluck('jenis_data')
            ->toArray();
        ...
        $selectedIndikator = $ropaActivity->riskIndicators->pluck('indikator')->toArray();
    @endphp
    ...
    @if ($errors->any())
        <div
            class="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 shadow-sm mb-4">
            ...
            <div class="p-6">
                ...
                <div id="tab-umum" class="tab-panel space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Kode
                                Aktivitas</label>
                        ...
                        @if (auth()->user()->hasRole(['Super Admin', 'admin']))
                            <div>
                                <label
                                    class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">OPD</label>
                                <select name="opd_id" {{ !$isEditable ? 'disabled' : '' }}
                                    ...
                                    @foreach ($opds as $opd)
                                        <option value="{{ $opd->id }}"
                                            {{ $ropaActivity->opd_id == $opd->id ? 'selected' : '' }}>{{ $opd->namaopd }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-span-2">
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nama
                                Aktivitas Pemrosesan <span class="text-red-500">*</span></label>
                            <input type="text" name="nama_aktivitas"
                                value="{{ old('nama_aktivitas', $ropaActivity->nama_aktivitas) }}"
                                {{ !$isEditable ? 'readonly' : '' }}
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 {{ !$isEditable ? 'bg-gray-50' : '' }}">
                        </div>
                        <div class="col-span-2">
                            <label
                                class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Penanggung
                                Jawab Proses <span class="text-red-500">*</span></label>
                            <input type="text" name="penanggung_jawab"
                                value="{{ old('penanggung_jawab', $ropaActivity->penanggung_jawab) }}"
                                {{ !$isEditable ? 'readonly' : '' }}
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 {{ !$isEditable ? 'bg-gray-50' : '' }}">
                        </div>
                        <div class="col-span-2">
                            <label
                                class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Deskripsi
                                & Tujuan Pemrosesan <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Proses
                                / Kegiatan Sebelumnya</label>
                            <input type="text" name="proses_sebelumnya"
                                value="{{ old('proses_sebelumnya', $ropaActivity->proses_sebelumnya) }}"
                                {{ !$isEditable ? 'readonly' : '' }}
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 {{ !$isEditable ? 'bg-gray-50' : '' }}">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Proses
                                / Kegiatan Setelahnya</label>
                            <input type="text" name="proses_setelahnya"
                                value="{{ old('proses_setelahnya', $ropaActivity->proses_setelahnya) }}"
                                {{ !$isEditable ? 'readonly' : '' }}
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 {{ !$isEditable ? 'bg-gray-50' : '' }}">
                        </div>
                        <div class="col-span-2">
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Sistem
                                / Aplikasi yang Digunakan</label>
                                    ...
                                    <div class="asset-row flex flex-wrap items-center gap-2">
                                        <select name="assets[{{ $loop->index }}][asset_instance_id]"
                                            ...
                                            @foreach ($assets as $asset)
                                                <option value="{{ $asset->id }}"
                                                    {{ $assetRel->asset_instance_id === $asset->id ? 'selected' : '' }}>
                                                    {{ $asset->kode_aset }} — {{ $asset->nama_aset }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <input type="text" name="assets[{{ $loop->index }}][nama_manual]"
                                            value="{{ $assetRel->nama_manual }}" placeholder="atau ketik nama manual..."
                                            {{ !$isEditable ? 'readonly' : '' }}
                                        ...
                                        <select name="assets[{{ $loop->index }}][peran_aset]"
                                            ...
                                            @foreach (\App\Models\RopaAsset::PERAN_LABELS as $val => $lbl)
                                                <option value="{{ $val }}"
                                                    {{ $assetRel->peran_aset === $val ? 'selected' : '' }}>
                                                    {{ $lbl }}</option>
                                            @endforeach
                                        </select>
                                        @if ($isEditable)
                        ...
                        <div class="col-span-2">
                            <label
                                class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Catatan /
                ...
                <div id="tab-data" class="tab-panel hidden space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Subjek
                                Data Pribadi <span class="text-red-500">*</span></label>
                            <input type="text" name="subjek_data"
                                value="{{ old('subjek_data', $ropaActivity->subjek_data) }}"
                                {{ !$isEditable ? 'readonly' : '' }} placeholder="Contoh: Masyarakat umum (pelapor)"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 {{ !$isEditable ? 'bg-gray-50' : '' }}">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Sumber
                                Pemerolehan Data <span class="text-red-500">*</span></label>
                            <input type="text" name="sumber_pemerolehan"
                                value="{{ old('sumber_pemerolehan', $ropaActivity->sumber_pemerolehan) }}"
                                {{ !$isEditable ? 'readonly' : '' }}
                    ...
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Data Pribadi Umum yang
                            Diproses</p>
                ...
                <div id="tab-penyimpanan" class="tab-panel hidden space-y-5">
                    <div>
                        <label
                            class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Penyimpanan
                            Data Pribadi</label>
                    ...
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Dasar Pemrosesan <span
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Referensi
                            Dasar Hukum</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Masa
                            Retensi Data Pribadi</label>
                ...
                <div id="tab-pengamanan" class="tab-panel hidden space-y-5">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Langkah
                            Teknis (Technical Safeguards)</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Langkah
                            Organisasi (Organisational Safeguards)</label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="text" name="recipients[{{ $loop->index }}][profil_penerima]"
                                            value="{{ $recipient->profil_penerima }}"
                                            {{ !$isEditable ? 'readonly' : '' }}
                                    ...
                                    <div>
                                        ...
                                        <select name="recipients[{{ $loop->index }}][tipe]"
                                            {{ !$isEditable ? 'disabled' : '' }} onchange="togglePeranField(this)"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                            <option value="internal"
                                                {{ $recipient->tipe === 'internal' ? 'selected' : '' }}>Internal</option>
                                            <option value="eksternal"
                                                {{ $recipient->tipe === 'eksternal' ? 'selected' : '' }}>Eksternal</option>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Tujuan
                                            Pengiriman</label>
                                        <input type="text" name="recipients[{{ $loop->index }}][tujuan_pengiriman]"
                                            value="{{ $recipient->tujuan_pengiriman }}"
                                            {{ !$isEditable ? 'readonly' : '' }}
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Mekanisme
                                            Pengiriman</label>
                                        <input type="text"
                                            name="recipients[{{ $loop->index }}][mekanisme_pengiriman]"
                                            value="{{ $recipient->mekanisme_pengiriman }}"
                                            {{ !$isEditable ? 'readonly' : '' }}
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div class="col-span-2">
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Jenis Data yang
                                            Dikirim</label>
                                        <input type="text" name="recipients[{{ $loop->index }}][jenis_data_dikirim]"
                                            value="{{ $recipient->jenis_data_dikirim }}"
                                            {{ !$isEditable ? 'readonly' : '' }}
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                </div>
                                @if ($isEditable)
                                    <div class="mt-2 flex justify-end">
                                        <button type="button" onclick="this.closest('.recipient-row').remove()"
                                            class="text-xs text-red-500 hover:text-red-700 font-medium">Hapus penerima
                                            ini</button>
                                    </div>
                                @endif
                            </div>
                ...
                <div id="tab-risiko" class="tab-panel hidden space-y-5">
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Hak Subjek Data
                            Pribadi yang Berlaku</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">
                            ...
                            <span class="text-xs font-normal text-red-500 ml-1">(Pasal 34 ayat 2 UU PDP)</span>
                        </p>
                        <p class="text-xs text-gray-400 mb-3">
                            Jika salah satu indikator terpenuhi, aktivitas ini <strong>wajib dilengkapi dengan
                                DPIA</strong>.
                        </p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            @foreach (\App\Models\RopaRiskIndicator::LABELS as $val => $label)
                                @php
                                    $isDataSpesifik = $val === 'data_spesifik';
                                @endphp
                                <label
                                    class="flex items-start gap-2 {{ $isDataSpesifik || !$isEditable ? 'opacity-75' : 'cursor-pointer' }}">
                                    <input type="checkbox" name="indikator_risiko[]" value="{{ $val }}"
                                        {{ in_array($val, old('indikator_risiko', $selectedIndikator)) ? 'checked' : '' }}
                                        {{ !$isEditable ? 'disabled' : '' }}
                                        {{ $isDataSpesifik ? 'id="indikator_data_spesifik"' : '' }}
                                        class="mt-0.5 h-4 w-4 rounded border-gray-300 text-red-500 focus:ring-red-500
                                           {{ $isDataSpesifik ? 'cursor-not-allowed' : '' }}">
                                    <span class="text-xs text-gray-700">
                                        {{ $label }}
                                        @if ($isDataSpesifik)
                                            <span class="text-gray-400 italic">(otomatis dari tab Data Pribadi)</span>
                                        @endif
                                    </span>
                                </label>
                            @endforeach
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Narasi
                            Asesmen Risiko</label>
    ...
    @if ($isEditable)
        <div id="modalHapus" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
            ...
            <div class="bg-white rounded-2xl border border-gray-300 shadow-2xl w-full max-w-md">
                <div class="px-6 py-5">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-gray-900">Hapus Aktivitas RoPA</h3>
                            <p class="text-xs text-gray-500 mt-0.5">Tindakan ini tidak dapat dibatalkan</p>
                        </div>
                    </div>
                    <p class="text-sm text-gray-600 mb-1">Anda akan menghapus aktivitas:</p>
                    <p class="text-sm font-semibold text-gray-900 mb-1">{{ $ropaActivity->kode }}</p>
                    <p class="text-xs text-gray-500 mb-5">{{ $ropaActivity->nama_aktivitas }}</p>
                    <p class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-5">
                        Semua data terkait (data pribadi, penerima, hak subjek, indikator risiko) akan ikut terhapus.
                    </p>
                    <div class="flex items-center justify-end gap-3">
                        ...
                        <form method="POST" action="{{ route('admin.ropa.destroy', $ropaActivity) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
