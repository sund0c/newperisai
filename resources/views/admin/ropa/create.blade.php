                ...
                <div id="tab-umum" class="tab-panel space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        @if (auth()->user()->hasRole(['Super Admin', 'admin']))
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">OPD
                                ...
                                <select name="opd_id"
                                    ...
                                    @foreach ($opds as $opd)
                                        <option value="{{ $opd->id }}"
                                            {{ old('opd_id') == $opd->id ? 'selected' : '' }}>{{ $opd->namaopd }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @else
                        ...
                        <div class="col-span-2">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Pilih dari inventaris aset yang sudah terdaftar, atau
                                ketik nama manual jika belum terdaftar.</p>
                        </div>
                        <div class="col-span-2">
                            <label
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            @foreach (\App\Models\RopaRiskIndicator::LABELS as $val => $label)
                                @php
                                    ...
                                    $autoChecked = $isDataSpesifik; // akan dikontrol via JS
                                @endphp
                                <label
                                    class="flex items-start gap-2 {{ $isDataSpesifik ? 'opacity-75' : 'cursor-pointer' }}">
                                    <input type="checkbox" name="indikator_risiko[]" value="{{ $val }}"
                                        {{ in_array($val, old('indikator_risiko', [])) ? 'checked' : '' }}
                                        {{ $isDataSpesifik ? 'id="indikator_data_spesifik" readonly' : '' }}
                                        class="mt-0.5 h-4 w-4 rounded border-gray-300 text-red-500 focus:ring-red-500
