...
@section('content')
    ...
    <nav class="flex mb-4 text-sm text-gray-500">
        ...
        <a href="{{ route('admin.master-kerawanan.class.show', $klas->id) }}"
            class="hover:text-blue-600">{{ $klas->klasifikasiaset }}</a>
        <span class="mx-2">/</span>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h2 class="text-sm font-semibold text-gray-800">Kerawanan Spesifik — {{ $subklas->subklasifikasiaset }}
                    </h2>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($displaySet->items->sortBy('nomor_urut') as $item)
                                <tr class="hover:bg-gray-50 transition-colors align-top">
                                    <td class="px-4 py-3 text-xs text-gray-400 font-mono">{{ $item->nomor_urut }}</td>
                                    <td class="px-4 py-3">
                                        <p class="text-xs font-mono text-gray-800">{{ $item->ancaman_tipikal ?? '—' }}</p>
                                    </td>
                                    <td class="px-4 py-3">
                                        <p class="text-xs font-mono text-gray-800">{{ $item->deskripsi }}</p>
                                    </td>
                                    <td class="px-4 py-3">
                                        <p class="text-xs font-mono text-gray-800">{{ $item->kategori ?? '—' }}</p>
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        @if ($item->mitigasi_tipikal)
                                            <p class="text-xs font-mono text-gray-800">{{ $item->mitigasi_tipikal }}</p>
                                        @else
                                            <span class="text-xs text-gray-300">—</span>
                                        @endif
                                    </td>
            ...
            @else
                <div class="px-6 py-12 text-center">
                    <p class="text-sm text-gray-400">Belum ada kerawanan spesifik untuk
                        {{ $subklas->subklasifikasiaset }}.</p>
    ...
    @if ($allVersions->count() > 0)
        <div id="modalRiwayat" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
            ...
            <div class="bg-white rounded-2xl border border-gray-300 shadow-2xl w-full max-w-lg">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 rounded-t-2xl flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-gray-800">Riwayat Versi — {{ $subklas->subklasifikasiaset }}
                    </h3>
