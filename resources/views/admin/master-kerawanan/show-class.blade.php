...
@section('title', 'Master Kerawanan — ' . $assetClass->nama)
@section('page-title', 'Master Kerawanan')
@section('page-subtitle', $assetClass->nama . ' — Kerawanan Global & Sub-kelas')
...
@section('content')
    ...
    <nav class="flex mb-4 text-sm text-gray-500" aria-label="Breadcrumb">
        ...
        <span class="text-gray-900 font-medium">{{ $assetClass->nama }}</span>
    </nav>

    {{-- Sub-kelas: horizontal chips --}}
    <div class="mb-4 flex flex-wrap gap-2">
        <span class="text-xs font-medium text-gray-400 self-center mr-1">Sub-kelas:</span>
        @foreach ($assetClass->subclasses as $sub)
            @php
                $subActive = $sub->activeVulnerabilitySet;
                $subCount = $subActive?->items()->count() ?? 0;
            @endphp
            <a href="{{ route('admin.master-kerawanan.subclass.show', [$assetClass, $sub]) }}"
                class="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-lg
                      text-xs font-medium text-gray-700 hover:border-blue-400 hover:text-blue-700
                      hover:bg-blue-50 transition-all shadow-sm group">
                <span class="font-mono text-gray-400 group-hover:text-blue-500">{{ $sub->kode }}</span>
                <span>{{ $sub->nama }}</span>
                @if ($subActive)
                    <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-green-100 text-green-700 text-xs">
                        <span class="w-1 h-1 rounded-full bg-green-500"></span>
                        v{{ $subActive->versi }} · {{ $subCount }}
                    </span>
                @else
                    <span class="px-1.5 py-0.5 rounded bg-amber-100 text-amber-600 text-xs">Belum ada</span>
                @endif
                <svg class="w-3 h-3 text-gray-300 group-hover:text-blue-400" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">
                        Berlaku untuk semua sub-kelas {{ $assetClass->kode }}
                        @if ($activeSet)
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if (!$draftSet)
                        <form method="POST" action="{{ route('admin.master-kerawanan.set.create-version') }}">
                            ...
                            <input type="hidden" name="scope_id" value="{{ $assetClass->id }}">
                            <button type="submit"
            ...
            @if ($displaySet && $displaySet->items->count() > 0)
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($displaySet->items as $item)
                            <tr class="hover:bg-gray-50 transition-colors" id="item-row-{{ $item->id }}">
                                <td class="px-6 py-3 text-xs text-gray-400 font-mono align-top">
                                    {{ $item->nomor_urut }}
                                </td>
                ...
                {{-- Tombol tambah item (hanya draft) --}}
                @if ($draftSet)
                ...
                <form action="{{ route('admin.master-kerawanan.item.store', $draftSet) }}" method="POST"
                    ...
                    <div>
                        ...
                        <textarea name="kontrol_tipikal" rows="2"
                            placeholder="Kontrol keamanan yang umumnya sudah ada (prediksi umum)..."
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm resize-none
                                     focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                    <div>
                        ...
                        <textarea name="mitigasi_tipikal" rows="3"
                            placeholder="Langkah mitigasi yang umumnya direkomendasikan untuk kerawanan ini..."
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm resize-none
                                     focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
                    </div>
                    <div class="flex items-center justify-end gap-3 pt-1">
    ...
    @if ($allVersions->count() > 0)
        <div id="modalRiwayat" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
            ...
            <div class="bg-white rounded-2xl border border-gray-300 shadow-2xl w-full max-w-lg">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 rounded-t-2xl flex items-center justify-between">
                    <h3 class="text-sm font-semibold text-gray-800">Riwayat Versi — Global {{ $assetClass->nama }}</h3>
    ...
    <script>
        // Publish modal — set action URL dinamis
        document.getElementById('publishSetId')?.addEventListener('change', function() {
            const form = document.getElementById('formPublish');
            form.action = `/admin/master-kerawanan/set/${this.value}/publish`;
        });
        ...
        function openEditModal(itemId, deskripsi, kontrol, rekomendasiAksi, catatanPlatform) {
            ...
            form.action = `/admin/master-kerawanan/items/${itemId}`;
        ...
        // Set publish form action saat publishSetId berubah
        const publishSetIdInput = document.getElementById('publishSetId');
        if (publishSetIdInput) {
            const observer = new MutationObserver(() => {
                const val = publishSetIdInput.value;
                if (val) document.getElementById('formPublish').action =
                    `/admin/master-kerawanan/set/${val}/publish`;
            });
            observer.observe(publishSetIdInput, {
                attributes: true,
