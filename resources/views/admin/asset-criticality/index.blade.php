...
@section('page-subtitle', 'Penilaian Confidentiality, Integrity & Availability (CIA) per aset · Tahun ' . ($tahunContext?->tahun ?? '-'))
...
<div class="mb-4">
    <div class="flex gap-3">
        ...
        <div class="flex-1 min-w-0 bg-white rounded-xl border border-gray-200 shadow-sm px-4 py-4">
            <div class="flex items-center justify-end mb-2">
                <span class="text-xs text-gray-400">
                    @if($totalAset > 0){{ round($totalTinggi / $totalAset * 100) }}%@endif
                </span>
            </div>
        ...
        <div class="flex-1 min-w-0 bg-white rounded-xl border border-gray-200 shadow-sm px-4 py-4">
            <div class="flex items-center justify-end mb-2">
                <span class="text-xs text-gray-400">
                    @if($totalAset > 0){{ round($totalSedang / $totalAset * 100) }}%@endif
                </span>
            </div>
        ...
        <div class="flex-1 min-w-0 bg-white rounded-xl border border-gray-200 shadow-sm px-4 py-4">
            <div class="flex items-center justify-end mb-2">
                <span class="text-xs text-gray-400">
                    @if($totalAset > 0){{ round($totalRendah / $totalAset * 100) }}%@endif
                </span>
            </div>
        ...
        <div class="px-6 py-4 flex items-center gap-3">
            ...
            <div class="w-40 shrink-0">
                <select name="kritikalitas"
                    ...
                    <option value="unassessed" {{ request('kritikalitas') === 'unassessed' ? 'selected' : '' }}>Belum Dinilai</option>
                </select>
            </div>
...
══════════════════════════════════════════════════════════ --}}
@if(session('success'))
<div class="flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800 shadow-sm mb-6">
...
<div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-36 whitespace-nowrap">
    ...
    <a href="{{ request()->fullUrlWithQuery(['sort' => 'kode_aset', 'direction' => $isSortKode && $direction === 'asc' ? 'desc' : 'asc']) }}"
       class="inline-flex items-center gap-1 hover:text-gray-700 transition-colors {{ $isSortKode ? 'text-blue-600' : '' }}">
        ...
        @endif
               ...
               <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">
    ...
    <a href="{{ request()->fullUrlWithQuery(['sort' => 'nama_aset', 'direction' => $isSortNama && $direction === 'asc' ? 'desc' : 'asc']) }}"
       class="inline-flex items-center gap-1 hover:text-gray-700 transition-colors {{ $isSortNama ? 'text-blue-600' : '' }}">
        ...
        @endif
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">OPD</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Klas / Sub Klas</th>
                ...
                <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-gray-500">Kritikal</th>
            </tr>
        </thead>
  ...
  <td class="px-3 py-3 text-xs text-gray-400">
                ...
                {{-- Kode Aset sebagai tombol trigger modal --}}
                ...
                <td class="px-6 py-3 max-w-[200px]">
                    ...
                    <div class="text-xs text-gray-400 mt-0.5 line-clamp-2">{{ $asset->keterangan ?? '-' }}</div>
                </td>
                ...
                <td class="px-4 py-3 text-center">
                    @if($c && isset($map[$c->confidentiality]))
                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-bold {{ $map[$c->confidentiality]['class'] }}"
                ...
                <td class="px-4 py-3 text-center">
                    @if($c && isset($map[$c->integrity]))
                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-bold {{ $map[$c->integrity]['class'] }}"
                ...
                <td class="px-4 py-3 text-center">
                    @if($c && isset($map[$c->availability]))
                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-bold {{ $map[$c->availability]['class'] }}"
...
{{-- ══════════════════════════════════════════════════════════
     MODAL EXPORT PDF — sama persis dengan menu aset
══════════════════════════════════════════════════════════ --}}
            ...
            <div>
                ...
                <select name="tahun" required
                    ...
                    @foreach($allTahun ?? [] as $t)
                        <option value="{{ $t->id }}" {{ $tahunContext?->id === $t->id ? 'selected' : '' }}>
                            {{ $t->tahun }}
                        </option>
                    @endforeach
                </select>
...
<div id="modalCIA"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col" style="max-height:90vh;">
        ...
        {{-- Header --}}
        <div class="flex-shrink-0 flex items-start justify-between rounded-t-2xl bg-gradient-to-r from-blue-700 to-blue-600 px-6 py-4">
            <div>
            ...
            <div class="grid grid-cols-3 divide-x divide-gray-100 overflow-y-auto flex-1">
                ...
                <div class="p-5 space-y-3">
                    <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-purple-100 text-sm font-bold text-purple-700">C</span>
                        <div>
                    ...
                    <label class="cia-option flex items-start gap-3 rounded-xl border-2 border-gray-200 p-3 cursor-pointer transition-all hover:border-purple-300 hover:bg-gray-50"
                        ...
                        <div>
                            ...
                            <p class="mt-1.5 text-xs text-gray-600 leading-relaxed">{{ Str::after($opt['label'], ': ') }}</p>
                        </div>
                    </label>
                    @endforeach
                </div>
                ...
                <div class="p-5 space-y-3">
                    <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-blue-100 text-sm font-bold text-blue-700">I</span>
                        <div>
                    ...
                    <label class="cia-option flex items-start gap-3 rounded-xl border-2 border-gray-200 p-3 cursor-pointer transition-all hover:border-blue-300 hover:bg-gray-50"
                ...
                <div class="p-5 space-y-3">
                    <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-green-100 text-sm font-bold text-green-700">A</span>
                        <div>
                    ...
                    <label class="cia-option flex items-start gap-3 rounded-xl border-2 border-gray-200 p-3 cursor-pointer transition-all hover:border-green-300 hover:bg-gray-50"
            ...
            <div class="flex-shrink-0 border-t border-gray-200 bg-gray-50 px-6 py-4 rounded-b-2xl flex items-center justify-between gap-4">
                <div>
                    <div id="previewWrap" class="hidden flex items-center gap-2">
                        ...
                        <span id="previewBadge" class="inline-flex items-center rounded-full px-3 py-1 text-sm font-bold"></span>
                        <span class="text-xs text-gray-400">= max(C, I, A)</span>
                    </div>
                    <p id="previewHint" class="text-xs text-gray-400 italic">Pilih nilai C, I, dan A untuk preview kritikalitas</p>
                </div>
...
const LEVEL_LABELS = { 1: 'Rendah', 2: 'Sedang', 3: 'Tinggi' };
const LEVEL_CLASSES = {
...
function setRadio(name, value) {
    ...
    if (radio) {
        radio.checked = true;
        highlightLabel(radio.closest('.cia-option'));
    }
}
...
function updatePreview() {
    ...
    if (c && i && a) {
        ...
        previewBadge.className    = 'inline-flex items-center rounded-full px-3 py-1 text-sm font-bold ' + LEVEL_CLASSES[max];
