        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    @endif
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Klas / Sub
                        Klas</th>
            ...
            <tbody class="divide-y divide-gray-100" id="assetTableBody">
                @forelse($assets as $i => $asset)
                    ...
                    <tr class="asset-row hover:bg-gray-50 transition-colors" data-id="{{ $asset->id }}"
                        ...
                        @if ($isAdmin)
                            <td class="px-6 py-3">
                                <div class="text-xs font-medium text-gray-700">{{ optional($asset->opd)->namaopd ?? '-' }}
                                </div>
                            </td>
                        @endif
                        ...
                        <td class="px-6 py-3">
                            <div class="text-xs font-medium text-gray-700">
                                {{ optional(optional($asset->subKlasifikasi)->klasifikasi)->klasifikasiaset ?? '-' }}</div>
                        ...
                        <td class="px-4 py-3 text-center">
                            @if ($se)
                                <span class="text-sm font-bold text-gray-700">{{ $se->total_nilai }}</span>
                ...
                @empty
                    <tr>
                        <td colspan="{{ $isAdmin ? 8 : 7 }}" class="px-6 py-12 text-center text-sm text-gray-400">
            ...
            <div
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-indigo-100">
                        Penilaian SE · Versi: <span class="font-semibold">{{ optional($seVersion)->kode ?? '—' }}</span>
                        ...
                        <div class="se-indikator-block rounded-xl border border-gray-200 overflow-hidden"
                            ...
                            <div class="flex items-start gap-3 px-4 py-3 bg-gray-50 border-b border-gray-100">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-gray-800 leading-snug">{{ $ind->pertanyaan }}</p>
                                    @if ($ind->keterangan)
                                    ...
                                    <label
                                        ...
                                        <div class="flex items-center gap-2">
                                            <input type="radio" name="se_ind_{{ $ind->id }}"
                                                ...
                                                onchange="onSERadioChange(this)" data-indikator-id="{{ $ind->id }}"
                                        ...
                                        <p class="text-[10px] text-gray-500 leading-snug pl-5">{{ $opt['pilihan'] }}</p>
                                    </label>
                                @endforeach
                            </div>
                        </div>
            ...
            <form id="formBulkSE" method="POST" action="{{ route('admin.asset-se.bulk-update') }}"
                ...
                <div
                    ...
                    <div>
                        ...
                        <p id="bulkSEPreviewHint" class="text-xs text-gray-400 italic">Jawab semua indikator untuk melihat
                            kategori</p>
                    </div>
                    <div class="flex gap-3 flex-shrink-0">
